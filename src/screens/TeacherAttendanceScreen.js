...
export default function TeacherAttendanceScreen({ route, navigation }) {
  ...
  const startClass = async () => {
    ...
    const [scheduledStartTime] = subject.time.split('-');
    const [hours, minutes] = scheduledStartTime.split(':').map(Number);
    const scheduledStart = new Date();
    scheduledStart.setHours(hours, minutes, 0, 0);
    
    const delayMinutes = Math.round((startTime - scheduledStart) / (1000 * 60));
    const isLate = delayMinutes > 5;
    ...
    if (isLate) {
      Alert.alert(
        'Late Start Alert',
        `Class started ${delayMinutes} minutes late.\nScheduled: ${subject.time}\nActual: ${startTime.toLocaleTimeString()}`,
        [{ text: 'OK' }]
      );
    }
    
    Alert.alert('Class Started', `${subject.name} session started successfully!`);
  };
  ...
  const saveAttendance = async () => {
    ...
    try {
      ...
      const filteredLogs = logs.filter(log => 
        !(log.subjectId === subject.id && log.date === date)
      );
      filteredLogs.push(attendanceData);
      await AsyncStorage.setItem('attendanceLogs', JSON.stringify(filteredLogs));
  ...
  const getAttendanceCount = () => {
    ...
    return { present, absent, late, total, unmarked: total - (present + absent + late) };
  };
  ...
  const getClassDuration = () => {
    ...
    const startTime = new Date(classSession.startTime);
    const endTime = classSession.endTime ? new Date(classSession.endTime) : currentTime;
    return Math.round((endTime - startTime) / (1000 * 60));
  };

  const renderStudent = ({ item }) => {
    const studentAttendance = attendance[item.id];
    const status = typeof studentAttendance === 'string' ? studentAttendance : studentAttendance?.status;
    ...
    <SafeAreaView style={styles.container}>
      ...
      <View style={styles.header}>
        ...
        <Text style={styles.dateHeader}>
          {date} • {currentTime.toLocaleTimeString('en-US', { hour12: false, hour: '2-digit', minute: '2-digit' })}
        </Text>
      </View>
      ...
      <View style={styles.sessionControls}>
        {!classSession ? (
          <TouchableOpacity style={styles.startClassButton} onPress={startClass}>
          ...
          <View style={styles.sessionInfo}>
            <View style={styles.sessionHeader}>
              <Text style={styles.sessionText}>
                ...
                {classSession.isLateStart && (
                  <Text style={styles.lateText}> ({classSession.delayMinutes}min late)</Text>
                )}
              </Text>
              {!classSession.endTime && (
                <TouchableOpacity style={styles.endClassButton} onPress={endClass}>
            ...
            <View style={styles.durationInfo}>
              <Text style={styles.durationText}>
                Duration: {currentDuration} min / {classSession.scheduledDuration} min
              </Text>
              <View style={styles.progressBar}>
                <View 
                  style={[
                    styles.progressFill,
                    { width: `${Math.min((currentDuration / classSession.scheduledDuration) * 100, 100)}%` }
                  ]} 
                />
              </View>
              <Text style={styles.efficiencyText}>
                Efficiency: {Math.round((currentDuration / classSession.scheduledDuration) * 100)}%
              </Text>
            </View>
              ...
              <View style={styles.completedSession}>
                ...
                <Text style={styles.finalStats}>
                  ...
                  {classSession.isShortClass && (
                    <Text style={styles.shortText}> • {classSession.shortfall}min short</Text>
                  )}
                </Text>
              </View>
            )}
