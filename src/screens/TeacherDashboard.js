    ...
    try {
      ...
      if (subjectsData) {
        ...
        const facultySubjects = allSubjects.filter(subject => 
          subject.faculty === faculty.facultyCode
        );
        setMySubjects(facultySubjects);

        // Get today's subjects
        const today = new Date().toLocaleDateString('en', { weekday: 'long' });
        const todaysClasses = facultySubjects.filter(subject => subject.day === today);
        setTodaySubjects(todaysClasses.sort((a, b) => a.time.localeCompare(b.time)));
      }
      ...
      >
        ...
        <View style={styles.header}>
          <View style={styles.welcomeSection}>
            ...
            <Text style={styles.facultyCode}>Faculty Code: {faculty.facultyCode}</Text>
          </View>
          ...
          {todaySubjects.length > 0 ? (
            todaySubjects.map((subject, index) => {
              ...
              return (
                <TouchableOpacity
                  key={index}
                  style={[styles.classCard, isCurrentTime && styles.currentClassCard]}
                ...
                >
                  <View style={styles.classInfo}>
                    ...
                    {isCurrentTime && (
                      <Text style={styles.currentIndicator}>NOW - Tap to start</Text>
                    )}
                  </View>
        ...
        <View style={styles.section}>
          ...
          <TouchableOpacity 
            ...
            onPress={() => navigation.navigate('TeacherSubjects', { facultyCode: faculty.facultyCode })}
          >
            ...
            <View key={index} style={styles.recentClassCard}>
              <View style={styles.recentClassInfo}>
                ...
                <Text style={styles.recentClassStats}>
                  Duration: {log.actualDuration || 'N/A'} min • 
                  Present: {Object.values(log.attendance).filter(a => 
